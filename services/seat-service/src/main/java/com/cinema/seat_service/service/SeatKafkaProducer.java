...
public class SeatKafkaProducer {
    @Value("${app.kafka.topic.seat_event}")
    ...
    public CompletableFuture<SendResult<String , SeatEvent>> publish(SeatEvent event
        ...
        CompletableFuture<SendResult<String , SeatEvent>> future=kafkaTemplate.send(topic  , event.getSeatId().toString() ,  event);

        future.whenComplete((result , ex) -> {
            if(ex != null){
                log.error("Failed to publish seat event for seatId: {} , showTimeId: {} , userId: {} , eventType: {}",
                        event.getSeatId() , event.getShowTimeId() , event.getUserId() , event.getEventType() , ex);
            }else{
                log.info("Successfully published seat event for seatId: {} , showTimeId: {} , userId: {} , eventType: {}",
                        event.getSeatId() , event.getShowTimeId() , event.getUserId() , event.getEventType());
            }
        });
